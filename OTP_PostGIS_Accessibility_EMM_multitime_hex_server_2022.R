...
#currently: grid_with_residents_20220609_30min
...
if (car == TRUE) {
  ...
  filename = paste0("grid_with_residents_20220609_",deptime,"_30min_car_hex") %>% str_remove(":")
...
points_car <- get_postgis_query(con, paste0("SELECT id, st_asgeojson(st_transform(
st_translate(
st_centroid(geom), random_between(0,100), random_between(0,100)), 4326))
                            AS centroid, st_asgeojson(st_transform(geom, 4326)) AS cell, 
                            residents_in_30min FROM ", filename,  " WHERE residents_in_30min = 0")) #%>% top_n(100)
...
FROM isochrones_car
WHERE ", filename, ".id = isochrones_car.id;")
...
filename <- "grid_with_residents_20220609_30min_bike_hex"
...
DROP TABLE IF EXISTS grid_with_residents_20220609_30min_bike_hex; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
SELECT g.id, g.geom, p.residents AS residents_in_30min
INTO grid_with_residents_20220609_30min_bike_hex
...
if (pt == TRUE) {
  ...
  filename = paste0("grid_with_residents_20220609_",deptime,"_30min_pt_hex") %>% str_remove(":")
...
-- DROP TABLE IF EXISTS grid_with_residents_20220609_30min_pt_hex2; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
-- SELECT g.id, g.geom, p.residents AS residents_in_30min
-- INTO grid_with_residents_20220609_30min_pt_hex2
--    FROM isochrones_pt AS p, hexgrid_1500 AS g
--    WHERE g.id = p.id;
...
############################# PT + Bike #############################
  ...
  filename = paste0("grid_with_residents_20220609_",deptime,"_30min_ptbike") %>% str_remove(":")
  
  # to calculate number of *inhabitants*
  processing_query_residents <- paste0("ALTER TABLE isochrones_ptbike ADD PRIMARY KEY (id); -- add PK
...
points_ptbike <- get_postgis_query(con, query) #%>% top_n(100)

# do this only, if points with 0 are found!
if (length(points_ptbike) > 0) {
...
UPDATE ", filename, "
SET residents_in_30min = isochrones_ptbike.residents
FROM isochrones_ptbike
WHERE ", filename, ".id = isochrones_ptbike.id;")
...
if (tweet == T) {try(updateStatus(paste0("PT+bike: ALL PostGIS operations done at ",  format(Sys.time(), "%X"))))}
...
if (bikeride == TRUE) {
  ...
  filename = paste0("grid_with_residents_20220609_",deptime,"_30min_bikeride_hex") %>% str_remove(":")
