...
#currently: grid_with_residents_20201029_30min
...
DROP TABLE IF EXISTS grid_with_residents_20201029_30min_car_hex; -- delete old table
...
INTO grid_with_residents_20201029_30min_car_hex
...
st_centroid(geom), random_between(0,100), random_between(0,100)), 4326))
                            ...
                            residents_in_30min FROM grid_with_residents_20201029_30min_car_hex 
...
UPDATE grid_with_residents_20201029_30min_car_hex
SET residents_in_30min = isochrones_car.residents
FROM isochrones_car
WHERE grid_with_residents_20201029_30min_car_hex.id = isochrones_car.id;"
...
############################# PT ############################# 
if (pt == TRUE) {
  if (tweet == T) { try(updateStatus(paste0("PT: starting script at ",  format(Sys.time(), "%X"))))}
  ...
  # to calculate number of *inhabitants*
  processing_query_residents <- "ALTER TABLE isochrones_pt ADD PRIMARY KEY (id); -- add PK
...
DROP TABLE IF EXISTS grid_with_residents_20201029_30min_pt_hex; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
SELECT g.id, g.geom, p.residents AS residents_in_30min
INTO grid_with_residents_20201029_30min_pt_hex
...
# Get *empty* and *shifted* centroid points from grid - import id, point, and grid cell
points_pt <- get_postgis_query(con, "SELECT id, st_asgeojson(st_transform(
st_translate(
st_centroid(geom), random_between(-750,750), random_between(-750,750)), 4326))
                            AS centroid, st_asgeojson(st_transform(geom, 4326)) AS cell, 
                            residents_in_30min FROM grid_with_residents_20201029_30min_pt_hex 
                               WHERE residents_in_30min = 0") #%>% top_n(100)
...
-- DROP TABLE IF EXISTS grid_with_residents_20201029_30min_pt_hex2; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
-- SELECT g.id, g.geom, p.residents AS residents_in_30min
-- INTO grid_with_residents_20201029_30min_pt_hex2
--    FROM isochrones_pt AS p, hexgrid_1500 AS g
--    WHERE g.id = p.id;

UPDATE grid_with_residents_20201029_30min_pt_hex
SET residents_in_30min = isochrones_pt.residents
FROM isochrones_pt
WHERE grid_with_residents_20201029_30min_pt_hex.id = isochrones_pt.id;"
...
if (tweet == T) {try(updateStatus(paste0("PT: PostGIS operations done at ",  format(Sys.time(), "%X"))))}
...
DROP TABLE IF EXISTS grid_with_residents_20201029_30min_bike_hex; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
SELECT g.id, g.geom, p.residents AS residents_in_30min
INTO grid_with_residents_20201029_30min_bike_hex
...
DROP TABLE IF EXISTS grid_with_residents_20201029_30min_ptbike_hex; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
SELECT g.id, g.geom, p.residents AS residents_in_30min
INTO grid_with_residents_20201029_30min_ptbike_hex
...
############################# Bike & Ride #############################
if (bikeride == TRUE) {
  if (tweet == T) {try(updateStatus(paste0("Bike+Ride: starting script at ",  format(Sys.time(), "%X"))))}
  ...
  # to calculate number of *inhabitants*
  processing_query_residents <- "ALTER TABLE isochrones_bikeride ADD PRIMARY KEY (id); -- add PK
...
DROP TABLE IF EXISTS grid_with_residents_20201029_30min_bikeride_hex; -- delete old table
-- JOIN: add residents within 30-min-isochrone to hexgrid_1500 in column residents_in_30min (by ID)
SELECT g.id, g.geom, p.residents AS residents_in_30min
INTO grid_with_residents_20201029_30min_bikeride_hex
    FROM isochrones_bikeride AS p, hexgrid_1500 AS g
    WHERE g.id = p.id;"

dbSendQuery(con, processing_query_residents)

if (tweet == T) { try(updateStatus(paste0("Bike+Ride: PostGIS operations done at ",  format(Sys.time(), "%X"))))}
}
